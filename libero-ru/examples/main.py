...
def eval_libero(args: Args) -> None:
    ...
    for task_id in tqdm.tqdm(range(num_tasks_in_suite)):
        ...
        for episode_idx in tqdm.tqdm(range(args.num_trials_per_task)):
            ...
            if not done:
                task_succ_arr.append(0)

            task_episodes += 1
            total_episodes += 1
            total_succ_arr.append(int(done))
            ...
            logging.info(
                f"# successes: {total_successes} ({total_successes / total_episodes * 100:.2f}%)"
            )
    ...
    logging.info(
        f"Total success rate: {float(total_successes) / float(total_episodes)} +- {np.std(total_succ_arr)}"
    )
